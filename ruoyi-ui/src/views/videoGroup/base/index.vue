<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="分组标题" prop="videoGroupName">
        <el-input
          v-model="queryParams.videoGroupName"
          placeholder="请输入视频分组标题"
          clearable
          size="small"
    ...
    <el-table v-loading="loading" :data="baseList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分组标题" align="center" prop="videoGroupName" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="视频分组标题" prop="videoGroupName">
          <el-input v-model="form.videoGroupName" placeholder="请输入视频分组标题" />
        </el-form-item>
...
<script>
import { listBase, getBase, delBase, addBase, updateBase } from "@/api/videoGroup/base";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除视频分组编号为"' + ids + '"的数据项？').then(function() {
        return delBase(ids);
      }).then(() => {
        this.getList();
